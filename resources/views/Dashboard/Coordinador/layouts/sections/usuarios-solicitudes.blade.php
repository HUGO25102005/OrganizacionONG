...
<div class="content w-full px-2 sm:px-4 lg:px-6">
    ...
    <div class="mt-2" id="lista">
        @if($datos and $estado != 0)
            {{ $datos->links() }}
        @endif
        @if ($beneficiariosearch2 and $estado == 0)
...
<script>
    ...
    const tbody = document.querySelector('#beneficiarioss-tbody');

    // Escucha cambios en el input de búsqueda
    searchInput.addEventListener('input', () => {
        const formData = new FormData(searchForm);
        const url = "{{ route('coordinador.beneficiarios.searchbs') }}";
