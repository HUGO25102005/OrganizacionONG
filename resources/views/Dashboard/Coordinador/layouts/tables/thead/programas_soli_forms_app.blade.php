<div class="bg-white p-4 rounded-lg shadow-md mb-6">
    ...
    <form action="{{ route('coordinador.programas', ['seccion' => 2]) }}" method="POST" class="flex flex-wrap items-center">
        @csrf
        @method('GET')
        ...
        <div class="w-full md:w-48 flex flex-col md:flex-row items-center md:space-x-4">
            ...
            <select id="estado" name="estado"
                ...
                <option value="1" {{ $estado == '1' ? 'selected' : '' }}>Solicitudes</option>
                <option value="3" {{ $estado == '3' ? 'selected' : '' }}>Aprobados</option>
                <option value="6" {{ $estado == '6' ? 'selected' : '' }}>Cancelados</option>
            </select>
        </div>
